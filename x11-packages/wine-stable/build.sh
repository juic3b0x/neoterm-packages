NEOTERM_PKG_HOMEPAGE=https://www.winehq.org/
NEOTERM_PKG_DESCRIPTION="A compatibility layer for running Windows programs"
...
NEOTERM_PKG_SRCURL=https://dl.winehq.org/wine/source/${NEOTERM_PKG_VERSION:0:3}/wine-$NEOTERM_PKG_VERSION.tar.xz
NEOTERM_PKG_SHA256=7cfd090a5395f5b76d95bb5defac8a312c8de4c070c1163b8b58da38330ca6ee
NEOTERM_PKG_DEPENDS="fontconfig, freetype, krb5, libandroid-spawn, libc++, libgmp, libgnutls, libxcb, libxcomposite, libxcursor, libxfixes, libxrender, mesa, opengl, pulseaudio, sdl2, vulkan-loader, xorg-xrandr"
NEOTERM_PKG_ANTI_BUILD_DEPENDS="vulkan-loader"
NEOTERM_PKG_BUILD_DEPENDS="libandroid-spawn-static, vulkan-loader-generic"
...
exec_prefix=$NEOTERM_PREFIX
--with-wine-tools=$NEOTERM_PKG_HOSTBUILD_DIR
--enable-nls
--disable-tests
--without-alsa
--without-capi
...
if [ "$NEOTERM_ARCH_BITS" = 64 ]; then
	NEOTERM_PKG_EXTRA_CONFIGURE_ARGS+=" --enable-win64"
fi

# Enable new WoW64 support on x86_64.
if [ "$NEOTERM_ARCH" = "x86_64" ]; then
	NEOTERM_PKG_EXTRA_CONFIGURE_ARGS+=" --enable-archs=i386,x86_64"
fi
...
_setup_llvm_mingw_toolchain() {
	...
	local _path="$NEOTERM_PKG_CACHEDIR/$(basename $_url)"
	local _sha256sum=9ae925f9b205a92318010a396170e69f74be179ff549200e8122d3845ca243b8
	neoterm_download $_url $_path $_sha256sum
	local _extract_path="$NEOTERM_PKG_CACHEDIR/llvm-mingw-toolchain-$_llvm_mingw_version"
...
neoterm_step_host_build() {
	...
	make -j "$NEOTERM_MAKE_PROCESSES" __tooldeps__ nls/all
}
