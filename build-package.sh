...
while (($# >= 1)); do
	case "$1" in
		...
		-i)
			...
			elif [ "$NEOTERM_PREFIX" != "/data/data/io.neoterm/files/usr" ]; then
				neoterm_error_exit "./build-package.sh: option '-i' is available only when NEOTERM_APP_PACKAGE is 'io.neoterm'"
			else
				export NEOTERM_INSTALL_DEPS=true
			fi
			;;
		-I)
			if [ "$NEOTERM_PREFIX" != "/data/data/io.neoterm/files/usr" ]; then
				neoterm_error_exit "./build-package.sh: option '-I' is available only when NEOTERM_APP_PACKAGE is 'io.neoterm'"
			else
				export NEOTERM_INSTALL_DEPS=true
