#!/usr/bin/env bash
# setup CGCT - Cross Gnu Compiler for NeoTerm
# compile glibc-based binaries for NeoTerm
...
declare -A CGCT=(
	["cbt"]="cbt-${VERSION_OF_CBT}-${ARCH}.pkg.tar.xz" # Cross Binutils for NeoTerm
	["cgt"]="cgt-${VERSION_OF_CGT}-${ARCH}.pkg.tar.xz" # Cross GCC for NeoTerm
)
