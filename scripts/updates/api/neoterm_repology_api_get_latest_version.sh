...
#
# But hopefully, all this can be avoided if NEOTERM_PKG_UPDATE_VERSION_REGEXP is set.
#
neoterm_repology_api_get_latest_version() {
	if [[ -z "$1" ]]; then
		neoterm_error_exit "Usage: ${FUNCNAME[0]} PKG_NAME"
	fi

	if [[ ! -s "${NEOTERM_REPOLOGY_DATA_FILE}" ]]; then
		...
		python3 "${NEOTERM_SCRIPTDIR}"/scripts/updates/api/dump-repology-data \
			"${NEOTERM_REPOLOGY_DATA_FILE}" >/dev/null
	fi

	local PKG_NAME="$1"
	local version
	# Why `--arg`? See: https://stackoverflow.com/a/54674832/15086226
	version="$(jq -r --arg packageName "$PKG_NAME" '.[$packageName]' <"${NEOTERM_REPOLOGY_DATA_FILE}")"
	echo "${version#v}"
}
