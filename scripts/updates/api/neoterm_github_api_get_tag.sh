...
neoterm_github_api_get_tag() {
	if [[ -z "$1" ]]; then
		neoterm_error_exit <<-EndOfUsage
	...
	if [[ -z "${GITHUB_TOKEN:-}" ]]; then
		...
		neoterm_error_exit "ERROR: GITHUB_TOKEN environment variable not set."
	fi
	...
	if [[ "${http_code}" == "200" ]]; then
		if [[ "${FILTER_REGEX}" ]]; then
			if jq --exit-status --raw-output "${jq_filter}" <<<"${response}" >/dev/null; then
				...
				if [[ -z "${tag_name}" ]]; then
					neoterm_error_exit "ERROR: No tags matched regex '${FILTER_REGEX}' in '${response}'"
				fi
			else
				neoterm_error_exit "ERROR: Failed to parse tag name from: '${response}'"
			fi
		else
			if jq --exit-status --raw-output "${jq_filter}" <<<"${response}" >/dev/null; then
				tag_name="$(jq --exit-status --raw-output "${jq_filter}" <<<"${response}")"
