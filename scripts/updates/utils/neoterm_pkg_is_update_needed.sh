...
neoterm_pkg_is_update_needed() {
	# USAGE: neoterm_pkg_is_update_needed <current-version> <latest-version>
	if [[ -z "$1" ]] || [[ -z "$2" ]]; then
		neoterm_error_exit "${BASH_SOURCE[0]}: at least 2 arguments expected"
	fi
	...
	neoterm_error_exit "Bad 'dpkg --compare-versions' exit code: $DPKG_EXIT_CODE - bad version numbers?"
}

# Make it also usable as command line tool. `scripts/bin/apt-compare-versions` is symlinked to this file.
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
	declare -f neoterm_error_exit >/dev/null ||
		. "$(dirname "$(realpath "${BASH_SOURCE[0]}")")/neoterm_error_exit.sh" # realpath is used to resolve symlinks.
	...
	if [[ -n "${version_regexp}" ]]; then
		...
		if [[ -z "${first_version}" ]] || [[ -z "${second_version}" ]]; then
			neoterm_error_exit "ERROR: Unable to parse version numbers using regexp '${version_regexp}'"
		fi
	fi
	if [[ "${first_version}" == "${second_version}" ]]; then
		echo "${first_version} = ${second_version}"
	else
		if neoterm_pkg_is_update_needed "${first_version}" "${second_version}"; then
			echo "${first_version} < ${second_version}"
		elif neoterm_pkg_is_update_needed "${second_version}" "${first_version}"; then
			echo "${first_version} > ${second_version}"
		else
			echo "${first_version} = ${second_version}"
		fi
