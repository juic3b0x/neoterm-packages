...
neoterm_download() {
	if [ $# != 3 ]; then
		neoterm_error_exit "neoterm_download(): Invalid arguments - expected \$URL \$DESTINATION \$CHECKSUM"
	fi
	local URL="$1"
	local DESTINATION="$2"
	local CHECKSUM="$3"
	...
	local TMPFILE
	TMPFILE=$(mktemp "$NEOTERM_PKG_TMPDIR/download.${NEOTERM_PKG_NAME-unnamed}.XXXXXXXXX")
