neoterm_setup_flang() {
    if [ "$NEOTERM_ON_DEVICE_BUILD" = true ]; then
		if [[ "$NEOTERM_APP_PACKAGE_MANAGER" = "apt" && "$(dpkg-query -W -f '${db:Status-Status}\n' flang 2>/dev/null)" != "installed" ]] ||
			[[ "$NEOTERM_APP_PACKAGE_MANAGER" = "pacman" && ! "$(pacman -Q flang 2>/dev/null)" ]]; then
	...
	local _flang_toolchain_cache_dir="$NEOTERM_COMMON_CACHEDIR/flang-toolchain-cache"
	...
	neoterm_download $_flang_aarch64_libs_url $_flang_aarch64_libs_file $_flang_aarch64_libs_checksum
	neoterm_download $_flang_toolchain_url $_flang_toolchain_file $_flang_toolchain_checksum
	neoterm_download $_flang_x86_64_libs_url $_flang_x86_64_libs_file $_flang_x86_64_libs_checksum
	neoterm_download $_clang_toolchain_url $_clang_toolchain_file $_clang_toolchain_checksum

	local _flang_toolchain_version=0

	local _neoterm_toolchain_name="$(basename "$NEOTERM_STANDALONE_TOOLCHAIN")"

	local FLANG_FOLDER=
	if [ "${NEOTERM_PACKAGES_OFFLINE-false}" = "true" ]; then
		FLANG_FOLDER="$NEOTERM_SCRIPTDIR/build-tools/$_neoterm_toolchain_name-flang-v$_flang_toolchain_version"
	else
		FLANG_FOLDER="$NEOTERM_COMMON_CACHEDIR/$_neoterm_toolchain_name-flang-v$_flang_toolchain_version"
	fi
	...
	if [ ! -d "$FLANG_FOLDER" ]; then
		...
		cp -Rf $NEOTERM_STANDALONE_TOOLCHAIN/sysroot $FLANG_FOLDER_TMP/
		...
		for host_plat in aarch64-linux-android armv7a-linux-androideabi i686-linux-android x86_64-linux-android; do
			...
			#!/usr/bin/env bash
			if [ "\$1" != "-cpp" ] && [ "\$1" != "-fc1" ]; then
				\`dirname \$0\`/flang-new --target=${host_plat}${NEOTERM_PKG_API_LEVEL} -D__ANDROID_API__=$NEOTERM_PKG_API_LEVEL "\$@"
			else
			...
			cp $FLANG_FOLDER_TMP/bin/${host_plat}-flang-new \
				$FLANG_FOLDER_TMP/bin/${host_plat}${NEOTERM_PKG_API_LEVEL}-flang-new
		done

		cp $FLANG_FOLDER_TMP/bin/armv7a-linux-androideabi-flang-new \
			$FLANG_FOLDER_TMP/bin/arm-linux-androideabi-flang-new
		cp $FLANG_FOLDER_TMP/bin/armv7a-linux-androideabi-flang-new \
			$FLANG_FOLDER_TMP/bin/arm-linux-androideabi${NEOTERM_PKG_API_LEVEL}-flang-new
	...
	export FC=$NEOTERM_HOST_PLATFORM-flang-new
	export FCFLAGS="--target=$CCNEOTERM_HOST_PLATFORM -D__ANDROID_API__=$NEOTERM_PKG_API_LEVEL"
}
