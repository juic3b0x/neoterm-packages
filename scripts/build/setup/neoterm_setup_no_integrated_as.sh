neoterm_setup_no_integrated_as() {
	if [ "$NEOTERM_ON_DEVICE_BUILD" = true ]; then
		if [[ "$NEOTERM_APP_PACKAGE_MANAGER" = "apt" && "$(dpkg-query -W -f '${db:Status-Status}\n' binutils-bin 2>/dev/null)" != "installed" ]] ||
			[[ "$NEOTERM_APP_PACKAGE_MANAGER" = "pacman" && ! "$(pacman -Q binutils-bin 2>/dev/null)" ]]; then
	...
	local binutils_cross_bin="$NEOTERM_PREFIX/opt/binutils/cross/$NEOTERM_HOST_PLATFORM/bin"
	if [ ! -x "$binutils_cross_bin/as" ]; then
		neoterm_error_exit "[${FUNCNAME[0]}]: Package 'binutils-cross' is not installed."
	fi

	local prefix="$NEOTERM_COMMON_CACHEDIR/no-integrated-as"
