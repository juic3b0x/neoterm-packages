neoterm_step_configure_haskell_build() {
	...
	HOST_FLAG="--host=${NEOTERM_HOST_PLATFORM}"
	if [[ ${NEOTERM_PKG_EXTRA_CONFIGURE_ARGS} != "${NEOTERM_PKG_EXTRA_CONFIGURE_ARGS/--host=/}" ]]; then
		HOST_FLAG=""
	fi

	LIBEXEC_FLAG="--libexecdir=${NEOTERM_PREFIX}/libexec"
	...
	if [[ ${NEOTERM_PKG_EXTRA_CONFIGURE_ARGS} != "${NEOTERM_PKG_EXTRA_CONFIGURE_ARGS/--disable-library-stripping=/}" ]] || [[ ${NEOTERM_DEBUG_BUILD} == true ]]; then
		LIB_STRIPPING=""
	fi

	EXECUTABLE_STRIPPING="--enable-executable-stripping"
	if [[ ${NEOTERM_PKG_EXTRA_CONFIGURE_ARGS} != "${NEOTERM_PKG_EXTRA_CONFIGURE_ARGS/--disable-executable-stripping=/}" ]] || [[ ${NEOTERM_DEBUG_BUILD} == true ]]; then
		EXECUTABLE_STRIPPING=""
	fi

	SPLIT_SECTIONS="--enable-split-sections"
	if [[ ${NEOTERM_PKG_EXTRA_CONFIGURE_ARGS} != "${NEOTERM_PKG_EXTRA_CONFIGURE_ARGS/--disable-split-sections=/}" ]]; then
		SPLIT_SECTIONS=""
	fi
	...
	# https://github.com/neoterm/neoterm-packages/issues/76
	AVOID_GNULIB=""
	...
	env $AVOID_GNULIB cabal configure \
		$NEOTERM_HASKELL_OPTIMISATION \
		--prefix="$NEOTERM_PREFIX" \
		--configure-option="$HOST_FLAG" \
		--ghc-options="-optl-Wl,-rpath,$NEOTERM_PREFIX/lib -optl-Wl,--enable-new-dtags" \
		--with-compiler="$(command -v ghc)" \
		--with-ghc-pkg="$(command -v ghc-pkg)" \
		--with-hsc2hs="$(command -v hsc2hs)" \
		--hsc2hs-option=--cross-compile \
		--extra-lib-dirs="$NEOTERM_PREFIX/lib" \
		--extra-include-dirs="$NEOTERM_PREFIX/include" \
		--with-ld="$LD" \
		--with-strip="$STRIP" \
		--with-ar="$AR" \
