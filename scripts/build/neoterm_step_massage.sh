neoterm_step_massage() {
	...
	if [ -e "share/mime/mime.cache" ]; then
		neoterm_error_exit "MIME cache found in package. Please disable \`update-mime-database\`."
	fi
	...
	if [ "$NEOTERM_PACKAGE_LIBRARY" = "bionic" ]; then
		if [ "$NEOTERM_PKG_NO_STRIP" != "true" ] && [ "$NEOTERM_DEBUG_BUILD" = "false" ]; then
		...
		if [ "$NEOTERM_PKG_NO_ELF_CLEANER" != "true" ]; then
			# Remove entries unsupported by Android's linker:
			find . \( -path "./bin/*" -o -path "./lib/*" -o -path "./libexec/*" -o -path "./opt/*" \) -type f -print0 | xargs -r -0 \
				"$NEOTERM_ELF_CLEANER" --api-level $NEOTERM_PKG_API_LEVEL
		fi
	fi

	local pattern=""
	for file in ${NEOTERM_PKG_NO_SHEBANG_FIX_FILES}; do
		if [[ -z "${pattern}" ]]; then
			pattern="${file}"
			continue
		fi
	...
	if [ "$NEOTERM_PKG_NO_SHEBANG_FIX" != "true" ]; then
		# Fix shebang paths:
		while IFS= read -r -d '' file; do
			...
			if head -c 100 "$file" | head -n 1 | grep -E "^#!.*/bin/.*" | grep -q -E -v -e "^#! ?/system" -e "^#! ?$NEOTERM_PREFIX_CLASSICAL"; then
				sed --follow-symlinks -i -E "1 s@^#\!(.*)/bin/(.*)@#\!$NEOTERM_PREFIX/bin/\2@" "$file"
			fi
	...
	test ! -z "$NEOTERM_PKG_RM_AFTER_INSTALL" && rm -Rf $NEOTERM_PKG_RM_AFTER_INSTALL
	...
	# share/doc/$NEOTERM_PKG_NAME/ as a license file is always
	# installed there.
	if [ "$(find . -path "./${ADDING_PREFIX}share/doc/$NEOTERM_PKG_NAME" -prune -o -type f -print | head -n1)" = "" ]; then
		if [ -f "$NEOTERM_PKG_SRCDIR"/configure.ac -o -f "$NEOTERM_PKG_SRCDIR"/configure.in ]; then
			neoterm_error_exit "No files in package. Maybe you need to run autoreconf -fi before configuring."
		else
			neoterm_error_exit "No files in package."
		fi
	fi
	...
	if [ -n "$HARDLINKS" ]; then
		if [ "$NEOTERM_PACKAGE_LIBRARY" = "bionic" ]; then
			neoterm_error_exit "Package contains hard links: $HARDLINKS"
			...
			for i in ${!hard_list[@]}; do
				...
				for j in ${hard_list[$i]}; do
					...
					ln -sf "${NEOTERM_PREFIX_CLASSICAL}/${root_file:2}" "${j}"
				done
			done
		fi
	fi
	...
	if [ -d "./${ADDING_PREFIX}man" ]; then
		neoterm_error_exit "Package contains directory \"\$PREFIX/man\" ($NEOTERM_PREFIX/man). Use \"\$PREFIX/share/man\" ($NEOTERM_PREFIX/share/man) instead."
	fi

	# Check for directory "$PREFIX/$PREFIX" which almost always indicates
	# packaging error.
	if [ -d "./${NEOTERM_PREFIX#/}" ]; then
		neoterm_error_exit "Package contains directory \"\$PREFIX/\$PREFIX\" ($NEOTERM_PREFIX/${NEOTERM_PREFIX#/})"
	fi
	...
	if [ -d "./${_python_deb_install_layout_dir}" ]; then
		neoterm_error_exit "Package contains directory \"\$PREFIX/${_python_deb_install_layout_dir}\" ($NEOTERM_PREFIX/${_python_deb_install_layout_dir})"
	fi
	...
	if [ "$NEOTERM_PACKAGE_LIBRARY" = "bionic" ] && [ -d "lib" ]; then
		...
		for lib in $(find lib -name "*.so"); do
			...
			if $READELF -s "$lib" | egrep "${grep_pattern}" &> /dev/null; then
				neoterm_error_exit "$lib contains undefined symbols:\n$($READELF -s "$lib" | egrep "${grep_pattern}")"
			fi
		done
	fi
	...
	if ! [[ $NEOTERM_PKG_NAME =~ ghc|ghc-libs ]]; then
...
# Local function called by neoterm_step_massage
create_grep_pattern() {
