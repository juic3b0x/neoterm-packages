NEOTERM_PKG_HOMEPAGE=https://www.rust-lang.org/
NEOTERM_PKG_DESCRIPTION="Systems programming language focused on safety, speed and concurrency"
NEOTERM_PKG_LICENSE="MIT"
NEOTERM_PKG_MAINTAINER="@neoterm"
NEOTERM_PKG_VERSION="1.76.0"
NEOTERM_PKG_SRCURL=https://static.rust-lang.org/dist/rustc-${NEOTERM_PKG_VERSION}-src.tar.xz
NEOTERM_PKG_SHA256=805482b436442a6786d270cacbab8f00529e06141b27b7fb01909b97ce4f3464
_LLVM_MAJOR_VERSION=$(. $NEOTERM_SCRIPTDIR/packages/libllvm/build.sh; echo $LLVM_MAJOR_VERSION)
_LLVM_MAJOR_VERSION_NEXT=$((_LLVM_MAJOR_VERSION + 1))
NEOTERM_PKG_DEPENDS="clang, libc++, libllvm (<< ${_LLVM_MAJOR_VERSION_NEXT}), lld, openssl, zlib"
NEOTERM_PKG_BUILD_DEPENDS="wasi-libc"
NEOTERM_PKG_NO_STATICSPLIT=true
NEOTERM_PKG_RM_AFTER_INSTALL="
bin/llc
bin/llvm-*
bin/opt
share/wasi-sysroot
"

neoterm_step_pre_configure() {
	neoterm_setup_cmake
	neoterm_setup_rust

	# default rust-std package to be installed
	NEOTERM_PKG_DEPENDS+=", rust-std-${CARGO_TARGET_NAME/_/-}"

	local p="${NEOTERM_PKG_BUILDER_DIR}/src-bootstrap-src-utils-cc_detect.rs.diff"
	echo "Applying $(basename "${p}")"
	sed "s|@NEOTERM_PKG_API_LEVEL@|${NEOTERM_PKG_API_LEVEL}|g" "${p}" \
		| patch --silent -p1

	export RUST_LIBDIR=$NEOTERM_PKG_BUILDDIR/_lib
	mkdir -p $RUST_LIBDIR

	export LLVM_VERSION=$(. $NEOTERM_SCRIPTDIR/packages/libllvm/build.sh; echo $NEOTERM_PKG_VERSION)
	export LZMA_VERSION=$(. $NEOTERM_SCRIPTDIR/packages/liblzma/build.sh; echo $NEOTERM_PKG_VERSION)

	# we can't use -L$PREFIX/lib since it breaks things but we need to link against libLLVM-9.so
	ln -fst "${RUST_LIBDIR}" \
		${NEOTERM_PREFIX}/lib/libLLVM-${LLVM_VERSION/.*/}.so \
		${NEOTERM_PREFIX}/lib/libLLVM-${LLVM_VERSION}.so

	# rust tries to find static library 'c++_shared'
	ln -sf $NEOTERM_STANDALONE_TOOLCHAIN/sysroot/usr/lib/$NEOTERM_HOST_PLATFORM/libc++_static.a \
		$RUST_LIBDIR/libc++_shared.a

	# https://github.com/neoterm/neoterm-packages/issues/18379
	# NDK r26 multiple ld.lld: error: undefined symbol: __cxa_*
	ln -fst "${RUST_LIBDIR}" "${NEOTERM_PREFIX}"/lib/libc++_shared.so

	# https://github.com/neoterm/neoterm-packages/issues/11640
	# https://github.com/neoterm/neoterm-packages/issues/11658
	# The build system somehow tries to link binaries against a wrong libc,
	# leading to build failures for arm and runtime errors for others.
	# The following command is equivalent to
	#	ln -sft $RUST_LIBDIR \
	#		$NEOTERM_STANDALONE_TOOLCHAIN/sysroot/usr/lib/$NEOTERM_HOST_PLATFORM/$NEOTERM_PKG_API_LEVEL/lib{c,dl}.so
	# but written in a future-proof manner.
	ln -sft $RUST_LIBDIR $(echo | $CC -x c - -Wl,-t -shared | grep '\.so$')

	# rust checks libs in PREFIX/lib. It then can't find libc.so and libdl.so because rust program doesn't
	# know where those are. Putting them temporarly in $PREFIX/lib prevents that failure
	mv $NEOTERM_PREFIX/lib/libtinfo.so.6 $NEOTERM_PREFIX/lib/libtinfo.so.6.tmp
	mv $NEOTERM_PREFIX/lib/libz.so.1 $NEOTERM_PREFIX/lib/libz.so.1.tmp
	mv $NEOTERM_PREFIX/lib/libz.so $NEOTERM_PREFIX/lib/libz.so.tmp
	mv $NEOTERM_PREFIX/lib/liblzma.so.$LZMA_VERSION $NEOTERM_PREFIX/lib/liblzma.so.tmp

	# https://github.com/neoterm/neoterm-packages/issues/11427
	# Fresh build conflict: liblzma -> rust
	# ld: error: /data/data/com.neoterm/files/usr/lib/liblzma.a(liblzma_la-common.o) is incompatible with elf64-x86-64
	mv "${NEOTERM_PREFIX}"/lib/liblzma.a{,.tmp} || :
}

neoterm_step_configure() {
	# it breaks building rust tools without doing this because it tries to find
	# ../lib from bin location:
	# this is about to get ugly but i have to make sure a rustc in a proper bin lib
	# configuration is used otherwise it fails a long time into the build...
	# like 30 to 40 + minutes ... so lets get it right

	# upstream tests build using versions N and N-1
	local BOOTSTRAP_VERSION="${NEOTERM_PKG_VERSION}"
	rustup install $BOOTSTRAP_VERSION
	rustup default $BOOTSTRAP_VERSION-x86_64-unknown-linux-gnu
	export PATH=$HOME/.rustup/toolchains/$BOOTSTRAP_VERSION-x86_64-unknown-linux-gnu/bin:$PATH
	local RUSTC=$(command -v rustc)
	local CARGO=$(command -v cargo)

	sed \
		-e "s|@NEOTERM_PREFIX@|${NEOTERM_PREFIX}|g" \
		-e "s|@NEOTERM_STANDALONE_TOOLCHAIN@|${NEOTERM_STANDALONE_TOOLCHAIN}|g" \
		-e "s|@CARGO_TARGET_NAME@|${CARGO_TARGET_NAME}|g" \
		-e "s|@RUSTC@|${RUSTC}|g" \
		-e "s|@CARGO@|${CARGO}|g" \
		"${NEOTERM_PKG_BUILDER_DIR}"/config.toml > config.toml

	local env_host=$(printf $CARGO_TARGET_NAME | tr a-z A-Z | sed s/-/_/g)
	export ${env_host}_OPENSSL_DIR=$NEOTERM_PREFIX
	export RUST_LIBDIR=$NEOTERM_PKG_BUILDDIR/_lib
	export CARGO_TARGET_${env_host}_RUSTFLAGS="-L${RUST_LIBDIR}"

	# x86_64: __lttf2
	case "${NEOTERM_ARCH}" in
	x86_64)
		export CARGO_TARGET_${env_host}_RUSTFLAGS+=" -C link-arg=$(${CC} -print-libgcc-file-name)" ;;
	esac

	# NDK r26
	export CARGO_TARGET_${env_host}_RUSTFLAGS+=" -C link-arg=-lc++_shared"

	export X86_64_UNKNOWN_LINUX_GNU_OPENSSL_LIB_DIR=/usr/lib/x86_64-linux-gnu
	export X86_64_UNKNOWN_LINUX_GNU_OPENSSL_INCLUDE_DIR=/usr/include
	export PKG_CONFIG_ALLOW_CROSS=1
	# for backtrace-sys
	export CC_x86_64_unknown_linux_gnu=gcc
	export CFLAGS_x86_64_unknown_linux_gnu="-O2"
}

neoterm_step_make() {
	:
}

neoterm_step_make_install() {
	unset CC CXX CPP LD CFLAGS CXXFLAGS CPPFLAGS LDFLAGS PKG_CONFIG RANLIB

	# remove version suffix: beta, nightly
	local NEOTERM_PKG_VERSION=${NEOTERM_PKG_VERSION//~*}

	$NEOTERM_PKG_SRCDIR/x.py install --stage 1 --target aarch64-linux-android
	$NEOTERM_PKG_SRCDIR/x.py install --stage 1 --target armv7-linux-androideabi
	$NEOTERM_PKG_SRCDIR/x.py install --stage 1 --target i686-linux-android
	$NEOTERM_PKG_SRCDIR/x.py install --stage 1 --target x86_64-linux-android
	$NEOTERM_PKG_SRCDIR/x.py install --stage 1 std --target wasm32-unknown-unknown
	$NEOTERM_PKG_SRCDIR/x.py install --stage 1 std --target wasm32-wasi

	$NEOTERM_PKG_SRCDIR/x.py dist rustc-dev --host $CARGO_TARGET_NAME --target aarch64-linux-android
	$NEOTERM_PKG_SRCDIR/x.py dist rustc-dev --host $CARGO_TARGET_NAME --target armv7-linux-androideabi
	$NEOTERM_PKG_SRCDIR/x.py dist rustc-dev --host $CARGO_TARGET_NAME --target i686-linux-android
	$NEOTERM_PKG_SRCDIR/x.py dist rustc-dev --host $CARGO_TARGET_NAME --target x86_64-linux-android
	$NEOTERM_PKG_SRCDIR/x.py dist rustc-dev --host $CARGO_TARGET_NAME --target wasm32-unknown-unknown
	$NEOTERM_PKG_SRCDIR/x.py dist rustc-dev --host $CARGO_TARGET_NAME --target wasm32-wasi

	tar xvf build/dist/rustc-dev-$NEOTERM_PKG_VERSION-$CARGO_TARGET_NAME.tar.gz
	./rustc-dev-$NEOTERM_PKG_VERSION-$CARGO_TARGET_NAME/install.sh --prefix=$NEOTERM_PREFIX

	cd "$NEOTERM_PREFIX/lib"
	rm -f libc.so libdl.so
	mv $NEOTERM_PREFIX/lib/libtinfo.so.6.tmp $NEOTERM_PREFIX/lib/libtinfo.so.6
	mv $NEOTERM_PREFIX/lib/libz.so.1.tmp $NEOTERM_PREFIX/lib/libz.so.1
	mv $NEOTERM_PREFIX/lib/libz.so.tmp $NEOTERM_PREFIX/lib/libz.so
	mv $NEOTERM_PREFIX/lib/liblzma.so.tmp $NEOTERM_PREFIX/lib/liblzma.so.$LZMA_VERSION
	mv "${NEOTERM_PREFIX}"/lib/liblzma.a{.tmp,} || :

	ln -fs rustlib/${CARGO_TARGET_NAME}/lib/*.so .
	ln -fs lld ${NEOTERM_PREFIX}/bin/rust-lld

	cd "$NEOTERM_PREFIX/lib/rustlib"
	rm -rf components \
		install.log \
		uninstall.sh \
		rust-installer-version \
		manifest-* \
		x86_64-unknown-linux-gnu

	cd "${NEOTERM_PREFIX}/lib/rustlib/${CARGO_TARGET_NAME}/lib"
	echo "INFO: ${NEOTERM_PKG_BUILDDIR}/rustlib-rlib.txt"
	ls *.rlib | tee "${NEOTERM_PKG_BUILDDIR}/rustlib-rlib.txt"

	echo "INFO: ${NEOTERM_PKG_BUILDDIR}/rustlib-so.txt"
	ls *.so | tee "${NEOTERM_PKG_BUILDDIR}/rustlib-so.txt"

	echo "INFO: ${NEOTERM_PKG_BUILDDIR}/rustc-dev-${NEOTERM_PKG_VERSION}-${CARGO_TARGET_NAME}/rustc-dev/manifest.in"
	cat "${NEOTERM_PKG_BUILDDIR}/rustc-dev-${NEOTERM_PKG_VERSION}-${CARGO_TARGET_NAME}/rustc-dev/manifest.in" | tee "${NEOTERM_PKG_BUILDDIR}/manifest.in"

	sed -e 's/^.....//' -i "${NEOTERM_PKG_BUILDDIR}/manifest.in"
	local _included=$(cat "${NEOTERM_PKG_BUILDDIR}/manifest.in")
	local _included_rlib=$(echo "${_included}" | grep '\.rlib$')
	local _included_so=$(echo "${_included}" | grep '\.so$')
	local _included=$(echo "${_included}" | grep -v "/rustc-src/")
	local _included=$(echo "${_included}" | grep -v '\.rlib$')
	local _included=$(echo "${_included}" | grep -v '\.so$')

	echo _rlib
	while IFS= read -r _rlib; do
		echo "${_rlib}"
		local _included_rlib=$(echo "${_included_rlib}" | grep -v "${_rlib}")
	done < "${NEOTERM_PKG_BUILDDIR}/rustlib-rlib.txt"
	echo _so
	while IFS= read -r _so; do
		echo "${_so}"
		local _included_so=$(echo "${_included_so}" | grep -v "${_so}")
	done < "${NEOTERM_PKG_BUILDDIR}/rustlib-so.txt"

	export _INCLUDED=$(echo -e "${_included}\n${_included_rlib}\n${_included_so}")
	echo -e "INFO: _INCLUDED:\n${_INCLUDED}"
}

neoterm_step_post_massage() {
	rm -f lib/libtinfo.so.6
	rm -f lib/libz.so
	rm -f lib/libz.so.1
	rm -f lib/liblzma.so.$LZMA_VERSION
	rm -f lib/liblzma.a
}
