...
NEOTERM_PKG_DEPENDS="arpack-ng, bzip2, fftw, fltk, fontconfig, freetype, glpk, glu, graphicsmagick, libcurl, libhdf5, libiconv, libopenblas, libsndfile, opengl, openssl, pcre, portaudio, qhull, qrupdate-ng, rapidjson, readline, suitesparse, sundials, zlib"
...
neoterm_step_post_get_source() {
	...
	local ver_x=$(. $NEOTERM_SCRIPTDIR/packages/octave/build.sh; echo ${NEOTERM_PKG_VERSION#*:})
	if [ "${ver_e}" != "${ver_x}" ]; then
